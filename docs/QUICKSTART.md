...
The ready `v0.4.6` public release connects native Windows OMP over authenticated local loopback
...
```powershell
git clone --branch v0.4.6 --depth 1 https://github.com/alphastorm/omp-ninfer.git
...
MODEL_URL=$(python3 -c \
  'import json; print(json.load(open("releases/v0.4.6/manifest.json"))["components"]["model"]["artifact_url"])')
MODEL_BYTES=$(python3 -c \
  'import json; print(json.load(open("releases/v0.4.6/manifest.json"))["components"]["model"]["artifact_bytes"])')
MODEL_SHA256=$(python3 -c \
  'import json; print(json.load(open("releases/v0.4.6/manifest.json"))["components"]["model"]["artifact_sha256"])')
