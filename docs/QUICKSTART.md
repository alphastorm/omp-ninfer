...
> [!IMPORTANT]
> **v0.8.1 has passed its four documented routes (24 steps) on the published components.**
> The accepted scope is RTX 5090 container host, macOS client, Windows client, and RTX 4090
> native Windows. RTX 3090 is deferred and is not a v0.8.1 install lane.
...
> release's commands. See [route acceptance](#v081-route-acceptance) for the recorded runs.
...
| --- | --- | --- | --- |
| RTX 5090 + Windows 11 / Docker Desktop WSL2 | **v0.8.1 route accepted; stock OMP 18.3.0** | [RTX 5090 container lane](#ready-route-native-windows-and-docker-desktop-wsl2) | A first OMP turn plus the documented pass/fail acceptance observations |
| RTX 4090 + native Windows | **v0.8.1 route accepted; stock OMP 18.3.0** | [RTX 4090 native lane](#native-windows-rtx-4090-release-lane) | The documented acceptance checks on the exact published package |
...
native Windows - passed against the published v0.8.1 components and the upstream OMP 18.3.0
client before the release was cut
([accepted receipts](../releases/v0.8.1/acceptance/documented-routes.json)). Each block below is
byte-identical to the block those runs executed.

## Verify the release before setup

The `v0.8.1` release composes native Windows OMP over authenticated local loopback
