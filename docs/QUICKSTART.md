...
> **v0.8.1: RTX 5090 · RTX 4090; faster decode**

**Get started with the exact lane for your GPU and runtime.**

> [!IMPORTANT]
> **v0.8.1 is a candidate until its four documented routes pass on the published components.**
> The scope is RTX 5090 container host, macOS client, Windows client, and RTX 4090 native
> Windows. RTX 3090 is deferred and is not a v0.8.1 install lane.
> The commands below require the published v0.8.1 release and its readiness check.
> Do not bypass `--require-ready` or mix one release's manifest with another
> release's commands. See [route acceptance](#v081-route-acceptance) for its status.
...
| --- | --- | --- | --- |
| RTX 5090 + Windows 11 / Docker Desktop WSL2 | **v0.8.1 candidate; route acceptance pending** | [RTX 5090 container lane](#ready-route-native-windows-and-docker-desktop-wsl2) | A first OMP turn plus the documented pass/fail acceptance observations |
| RTX 4090 + native Windows | **v0.8.1 candidate; route acceptance pending** | [RTX 4090 native lane](#native-windows-rtx-4090-release-lane) | The documented acceptance checks on the exact published package |
...
with the v0.7.2 manifest and OMP 18.0.9. Do not combine them with v0.8.1 or OMP 18.3.0.
...
## v0.8.1 route acceptance

The four documented routes - RTX 5090 container host, macOS client, Windows client, and RTX 4090
native Windows - run against the published v0.8.1 components and the upstream OMP 18.3.0 client
before the release is cut. Until they pass, the release stays a candidate and `--require-ready`
refuses it. The v0.8.0 routes'
[accepted receipts](../releases/v0.8.0/acceptance/documented-routes.json) cover the v0.8.0
runtime, not these components.

## Verify the release before setup

The `v0.8.1` candidate composes native Windows OMP over authenticated local loopback
...
The published runtime components are RTX 5090 `v0.6.10-qwen38-5090-beta.1` from source
`8cc0810acc296bac482187da171afddd93df7fb9` and RTX 4090 `v0.6.8-qwen38-4090-beta.1` from source
`5a774841c29bdf2ee6b7efba135aed0a47e80447`. They decode faster than v0.8.0's components with
unchanged outputs: the MTP3 verify pass's small quantized projections share each activation load
across weight rows, and the Q4 MLP gate/up kernel no longer conflicts on shared memory. RTX 5090
decode is 10.3-11.0% faster from a seed context to 31K tokens; the RTX 4090, which keeps one
weight row per warp in two projections where sharing was slower, decodes its C1 benchmark at
157.89 tok/s instead of 153.54 ([EXP-055](measurements/2026-09-25-decode-kernel-schedules.json)).
...
full prefill, and their median time to first token was 0.094-0.101 s
...
sessions saved by v0.8.0 are not restored after the upgrade: OMP resends the full conversation and
each session re-prefills once.

Model, serving settings and memory floors are unchanged from v0.8.0. The public RTX 5090
...
`qwen38-4090-native-v0.6.8-beta.1`. A managed RTX 4090 start could fail when the driver refused to
pin the host-KV pool ([#48](https://github.com/alphastorm/omp-ninfer/issues/48)); since v0.8.0
the package commits each pinned allocation's size plus 1/64 before pinning it, and every managed
start in this package's canonical qualification pinned the pool. A refusal that still occurs
...
```powershell
git clone --branch v0.8.1 --depth 1 https://github.com/alphastorm/omp-ninfer.git
...
Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force
$ErrorActionPreference = 'Stop'
$Manifest = Get-Content .\releases\v0.8.1\manifest.json -Raw | ConvertFrom-Json
...
Once v0.8.1 is published and ready, run this on the Mac and inference host:
...
MODEL_URL=$(python3 -c \
  'import json; print(json.load(open("releases/v0.8.1/manifest.json"))["components"]["model"]["artifact_url"])')
MODEL_BYTES=$(python3 -c \
  'import json; print(json.load(open("releases/v0.8.1/manifest.json"))["components"]["model"]["artifact_bytes"])')
MODEL_SHA256=$(python3 -c \
  'import json; print(json.load(open("releases/v0.8.1/manifest.json"))["components"]["model"]["artifact_sha256"])')
...
RTX 3090 scout role is deferred with its GPU: it is not a v0.8.1 install lane, so neither the
