...
The ready `v0.6.7` public release connects native Windows OMP over authenticated local loopback
...
```powershell
git clone --branch v0.6.7 --depth 1 https://github.com/alphastorm/omp-ninfer.git
...
$Manifest = Get-Content .\releases\v0.6.7\manifest.json -Raw | ConvertFrom-Json
...
```sh
git clone --branch v0.6.7 --depth 1 \
...
MODEL_URL=$(python3 -c \
  'import json; print(json.load(open("releases/v0.6.7/manifest.json"))["components"]["model"]["artifact_url"])')
MODEL_BYTES=$(python3 -c \
  'import json; print(json.load(open("releases/v0.6.7/manifest.json"))["components"]["model"]["artifact_bytes"])')
MODEL_SHA256=$(python3 -c \
  'import json; print(json.load(open("releases/v0.6.7/manifest.json"))["components"]["model"]["artifact_sha256"])')
