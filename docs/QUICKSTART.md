...
The ready `v0.5.1` public release connects native Windows OMP over authenticated local loopback
...
MODEL_URL=$(python3 -c \
  'import json; print(json.load(open("releases/v0.5.1/manifest.json"))["components"]["model"]["artifact_url"])')
MODEL_BYTES=$(python3 -c \
  'import json; print(json.load(open("releases/v0.5.1/manifest.json"))["components"]["model"]["artifact_bytes"])')
MODEL_SHA256=$(python3 -c \
  'import json; print(json.load(open("releases/v0.5.1/manifest.json"))["components"]["model"]["artifact_sha256"])')
