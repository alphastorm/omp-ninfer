...
> **v0.8.2: RTX 5090 · RTX 4090; GPU keep-warm**

**Get started with the exact lane for your GPU and runtime.**

> [!IMPORTANT]
> **v0.8.2 is a candidate until its four documented routes pass on the published components.**
> The scope is RTX 5090 container host, macOS client, Windows client, and RTX 4090 native
> Windows. RTX 3090 is deferred and is not a v0.8.2 install lane.
> The commands below require the published v0.8.2 release and its readiness check.
> Do not bypass `--require-ready` or mix one release's manifest with another
> release's commands. See [route acceptance](#v082-route-acceptance) for its status.
...
| --- | --- | --- | --- |
| RTX 5090 + Windows 11 / Docker Desktop WSL2 | **v0.8.2 candidate; route acceptance pending** | [RTX 5090 container lane](#ready-route-native-windows-and-docker-desktop-wsl2) | A first OMP turn plus the documented pass/fail acceptance observations |
| RTX 4090 + native Windows | **v0.8.2 candidate; route acceptance pending** | [RTX 4090 native lane](#native-windows-rtx-4090-release-lane) | The documented acceptance checks on the exact published package |
...
with the v0.7.2 manifest and OMP 18.0.9. Do not combine them with v0.8.2 or OMP 18.3.0.
...
## v0.8.2 route acceptance

The four documented routes - RTX 5090 container host, macOS client, Windows client, and RTX 4090
native Windows - run against the published v0.8.2 components and the upstream OMP 18.3.0 client
before the release is cut. Until they pass, the release stays a candidate and `--require-ready`
refuses it. The v0.8.1 routes'
[accepted receipts](../releases/v0.8.1/acceptance/documented-routes.json) cover the v0.8.1
runtime, not these components.

## Verify the release before setup

The `v0.8.2` candidate composes native Windows OMP over authenticated local loopback
...
The published runtime components are RTX 5090 `v0.6.11-qwen38-5090-beta.1` from source
`32c21f73a7605f76480a6139de0488a14ed1aa48` and RTX 4090 `v0.6.8-qwen38-4090-beta.1` from source
`5a774841c29bdf2ee6b7efba135aed0a47e80447`, unchanged from v0.8.1. After its last work the RTX
5090 steps down to its lowest idle clocks within about 9 s, and the first prefill after that runs
up to 2.3x slower while the clocks ramp. The RTX 5090 runtime now has `--gpu-keep-warm-ms`, which
the v0.8.2 profile sets to 60000: for 60 s after the server goes idle, a single-warp kernel that
touches no memory spins 3.5 ms of every 10 ms on its own stream, and it stops when a request is
admitted. A new session arriving after 12-58 s of idle then prefilled in 0.155-0.157 s instead of
0.253-0.304 s (time to first token 0.173-0.181 s instead of 0.316-0.366 s), outputs stayed
byte-identical, and holding the clocks drew about 71 W above idle while it ran
...
full prefill, and their median time to first token was 0.090-0.098 s
([lane receipt](../releases/v0.8.2/qualification/rtx5090.json)).
...
sessions saved by v0.8.1 are not restored after the upgrade: OMP resends the full conversation and
each session re-prefills once.

The model and memory floors are unchanged from v0.8.1. The RTX 5090 serving profile adds
`--gpu-keep-warm-ms 60000`, so its deployment profile advances to `qwen38-5090-v0.8.2` /
configuration `56878aed`, with 16384 MiB host KV and a 28672 MiB runtime-host floor. The native
RTX 4090 keeps 11264 MiB host KV, 24 host-state slots, its 32768 MiB host floor and its release
identity `qwen38-4090-native-v0.6.8-beta.1`. A managed RTX 4090 start could fail when the driver
...
```powershell
git clone --branch v0.8.2 --depth 1 https://github.com/alphastorm/omp-ninfer.git
...
Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force
$ErrorActionPreference = 'Stop'
$Manifest = Get-Content .\releases\v0.8.2\manifest.json -Raw | ConvertFrom-Json
...
Once v0.8.2 is published and ready, run this on the Mac and inference host:
...
MODEL_URL=$(python3 -c \
  'import json; print(json.load(open("releases/v0.8.2/manifest.json"))["components"]["model"]["artifact_url"])')
MODEL_BYTES=$(python3 -c \
  'import json; print(json.load(open("releases/v0.8.2/manifest.json"))["components"]["model"]["artifact_bytes"])')
MODEL_SHA256=$(python3 -c \
  'import json; print(json.load(open("releases/v0.8.2/manifest.json"))["components"]["model"]["artifact_sha256"])')
...
RTX 3090 scout role is deferred with its GPU: it is not a v0.8.2 install lane, so neither the
