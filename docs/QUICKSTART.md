...
The ready `v0.3.1` public release connects native Windows OMP over authenticated local loopback
...
```powershell
git clone --branch v0.3.1 --depth 1 https://github.com/alphastorm/omp-ninfer.git
...
```powershell
$Manifest = Get-Content .\releases\v0.3.1\manifest.json -Raw | ConvertFrom-Json
...
MODEL_URL=$(python3 -c \
  'import json; print(json.load(open("releases/v0.3.1/manifest.json"))["components"]["model"]["artifact_url"])')
MODEL_BYTES=$(python3 -c \
  'import json; print(json.load(open("releases/v0.3.1/manifest.json"))["components"]["model"]["artifact_bytes"])')
MODEL_SHA256=$(python3 -c \
  'import json; print(json.load(open("releases/v0.3.1/manifest.json"))["components"]["model"]["artifact_sha256"])')
