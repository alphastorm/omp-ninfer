...
| EXP-024 | RTX 5090 requalification (v0.5.1 candidate) | The warm-arrival and restore changes hold under the lane's own qualification gates on the canonical build | Appliance-local build of `d956e6d6` (binary `71edc2f6`, 9/9 host tests, packaged with SBOM) on the unchanged v0.4.8 arguments: exact 130,048-token retrieval at 2,196 tok/s cold (v0.4.8: 2,207), 2,048-token decode at 138.2 tok/s with 41.2% MTP acceptance (136.0), agent protocol with no resurrection across restart; fanout 24/24 forks on `private_long_anchor` across three template sizes (57.9K, 67.7K, 80.0K) in-process and after a verified restart with the resume first (post-restart forks 1.24-1.42 s; v0.4.8 paid 22.2 s on the first); 4.5 GB explicit save 4.7 s (was 12.6 s, the writer's digest also runs on the SHA extensions) | passed - staged for `v0.5.1`; publication blocked on the archive release, runtime-image workflow, and external acceptance |
