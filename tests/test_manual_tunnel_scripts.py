...
class ManualTunnelScriptsTest(unittest.TestCase):
    def test_start_identity_uses_profile_deployment_identity(self) -> None:
        ...
        self.assertIn('MANIFEST="$ROOT/releases/v0.6.2/manifest.json"', source)
        ...
        self.assertIn("EXPECTED_RELEASE=v0.6.2", stop_source)
    ...
    def test_native_lane_install_is_completable_from_the_document(self) -> None:
        ...
        manifest = json.loads(
            (ROOT / "releases" / "v0.6.2" / "manifest.json").read_text(encoding="utf-8")
        )
